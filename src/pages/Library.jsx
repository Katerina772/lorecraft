...
export default function Library() {
  ...
  useEffect(() => {
    const loadGenres = async () => {
      ...
      if (genreParam) {
        ...
        if (genre) setSelectedGenres([genre.id]);
      } else if (genresParam) {
        const ids = genresParam.split(",").map(Number);
  ...
  // Завантаження всіх опублікованих квестів
  useEffect(() => {
  ...
  // Синхронізація URL при зміні вибраних жанрів
  useEffect(() => {
    if (selectedGenres.length === 0) {
    ...
    const names = selectedGenres
      ...
      .filter(Boolean);
    if (names.length === 1) {
      setSearchParams({ genre: names[0] }, { replace: true });
    } else {
      setSearchParams({ genres: names.join(",") }, { replace: true });
    }
  }, [selectedGenres, genres, setSearchParams]);
  ...
  const filteredQuests = useMemo(() => {
    ...
    if (searchAuthor.trim()) {
      const query = searchAuthor.toLowerCase();
      result = result.filter((q) => q.authorName.toLowerCase().includes(query));
    }

    if (selectedGenres.length > 0) {
      result = result.filter((q) => selectedGenres.includes(q.genreId));
    }

    // Категорії (умовно)
    if (activeCategory === "Popular") {
      result = result.filter((q) => q.playCount > 1000);
    } else if (activeCategory === "New") {
      const now = new Date();
      const weekAgo = new Date(now - 7 * 24 * 60 * 60 * 1000);
      result = result.filter((q) => new Date(q.createdAt) > weekAgo);
    } else if (activeCategory === "Top Rated") {
      result = result.filter((q) => q.averageRating >= 4.5);
    }
    ...
    return result;
  }, [quests, searchTitle, searchAuthor, selectedGenres, activeCategory, sort]);
