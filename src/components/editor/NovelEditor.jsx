...
//     };
//     // Зберігаємо у списку чернеток поточного користувача
//     const drafts = JSON.parse(
//       localStorage.getItem(`drafts_${user.id}`) || "[]",
//     );
//     // Якщо квест вже має id (редагування існуючої чернетки), оновити його
//     const existingIndex = drafts.findIndex(
//       (d) => d.meta.title === questMeta.title,
//     );
//     if (existingIndex >= 0) {
...
//   const publish = () => {
//     if (!user) return;
//     // Створюємо новий квест зі структурою, як у бібліотеці
//     const newQuest = {
...
//       scenes: scenes, // додаємо сцени для проходження
//       authorId: user.id,
//       publishedAt: new Date().toISOString(),
//     };

//     // Додаємо до глобального списку опублікованих квестів
...
//     // Додаємо id квесту до списку створених користувачем
...
//     navigate("/profile"); // переходимо у профіль
//   };
          ...
          <div className="w-56 border-r border-primary/20 p-3 bg-card overflow-y-auto">
            ...
            {scenes.map((s) => (
              <div
                ...
                onClick={() => setSelectedId(s.id)}
                className={`flex justify-between items-center px-3 py-2 rounded-lg mb-1 cursor-pointer text-sm font-medium transition-colors group ${
                  s.id === selectedId
                    ? "bg-primary/30 border border-button"
                    : "hover:bg-primary/10 border border-transparent"
                }`}
